...
function createLocalServer({ runner, publicDirectory, memory, authToken = '' }) {
  const expectedToken = typeof authToken === 'string' ? authToken : '';
  const server = http.createServer(async (request, response) => {
    try {
      const requestUrl = new URL(request.url || '/', 'http://127.0.0.1');
      const pathname = requestUrl.pathname;

      if (pathname.startsWith('/api/') && expectedToken
        && request.headers['x-monitor-token'] !== expectedToken) {
        writeJson(response, 401, { error: 'Unauthorized local app request.' });
        return;
      }
      ...
      if (pathname === '/api/displays' && request.method === 'GET') {
        const displays = typeof runner.refreshDisplays === 'function'
          ? runner.refreshDisplays()
          : runner.snapshot().displays;
      ...
      if (pathname === '/api/analyze' && request.method === 'POST') {
        ...
        const result = typeof runner.requestAnalysis === 'function'
          ? runner.requestAnalysis({ reason: body.reason || 'button', promptOverride: body.promptOverride || '' })
          : { accepted: false, reason: 'analysis-unavailable' };
      ...
      if (pathname === '/api/memory' && request.method === 'GET') {
        writeJson(response, 200, memory.details ? memory.details() : memory.summary());
        return;
      }
