...
const DEFAULT_COMBINED_PROMPT = [
  'Answer the spoken question using the current screen as additional context when relevant.',
  'If the screen contains a coding exercise, explain the approach and provide the requested solution.',
...
const DEFAULT_SETTINGS = Object.freeze({
  ...
  voiceCustomModel: '',
  combinedModel: 'gpt-5.6-luna',
  combinedCustomModel: '',
  reasoning: 'medium',
  imageDetail: 'auto',
  combinedReasoning: 'medium',
  combinedImageDetail: 'auto',
  ...
  combinedPrompt: DEFAULT_COMBINED_PROMPT,
...
const ALLOWED = {
  ...
  combinedModel: MODEL_OPTIONS,
  reasoning: new Set(['none', 'low', 'medium', 'high', 'xhigh', 'max']),
  imageDetail: new Set(['auto', 'low', 'high', 'original']),
  combinedReasoning: new Set(['none', 'low', 'medium', 'high', 'xhigh', 'max']),
  combinedImageDetail: new Set(['auto', 'low', 'high', 'original']),
...
function normalizeSettings(input = {}, base = DEFAULT_SETTINGS) {
  ...
  const combinedModel = asEnum(source.combinedModel, ALLOWED.combinedModel, defaults.combinedModel);
  ...
  return {
    ...
    combinedModel,
    combinedCustomModel: typeof source.combinedCustomModel === 'string'
      ? source.combinedCustomModel.trim().slice(0, 200)
      : defaults.combinedCustomModel,
    ...
    combinedReasoning: asEnum(source.combinedReasoning, ALLOWED.combinedReasoning, defaults.combinedReasoning),
    combinedImageDetail: asEnum(source.combinedImageDetail, ALLOWED.combinedImageDetail, defaults.combinedImageDetail),
    ...
    combinedAnswerLanguage: typeof source.combinedAnswerLanguage === 'string' && source.combinedAnswerLanguage.trim()
      ? source.combinedAnswerLanguage.trim().slice(0, 80)
      : defaults.combinedAnswerLanguage,
    combinedPrompt: typeof source.combinedPrompt === 'string' && source.combinedPrompt.trim()
      ? source.combinedPrompt.slice(0, 10000)
      : defaults.combinedPrompt,
