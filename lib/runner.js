...
class MonitorRunner {
  ...
  async processVoiceAnswerQueue() {
    ...
    try {
      ...
      const createdAt = memoryEntry?.createdAt || baseline.completedAt || memoryAnswer.completedAt || combinedAnswer.completedAt || now();
      ...
      if (combinedAnswer.answer) {
        this.state.combined = {
          ...this.state.combined,
          history: appendHistory(this.state.combined.history, {
            id,
            createdAt,
            transcript,
            prompt: settings.voicePrompt,
            answer: combinedAnswer.answer,
            sourceDisplayNumber: combinedAnswer.sourceDisplayNumber || null,
            sourceLabel: combinedAnswer.sourceLabel || '',
            captureAt: combinedAnswer.captureAt || null
          })
        };
        historyChanged = true;
      }
