...
class MonitorRunner {
  ...
  setDisplays(displays) {
    ...
    const exactSource = this.displays.find((display) => savedSourceId && String(display.id) === savedSourceId)
  ...
  findSourceDisplay() {
    return this.displays.find((display) => String(display.id) === String(this.settings.sourceDisplayId || ''))
      || this.displays.find((display) => display.captureNumber === this.settings.sourceDisplayNumber)
      || this.displays[0]
      || null;
  }
