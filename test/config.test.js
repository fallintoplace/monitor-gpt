...
const { DEFAULT_COMBINED_PROMPT, DEFAULT_PROMPT, DEFAULT_SETTINGS, loadSettings, normalizeSettings, saveSettings } = require('../lib/config');
...
test('should keep the remembered defaults when settings are missing', () => {
  ...
  assert.equal(settings.combinedModel, 'gpt-5.6-luna');
  assert.equal(settings.reasoning, 'medium');
  assert.equal(settings.combinedReasoning, 'medium');
  assert.equal(settings.combinedImageDetail, 'auto');
  ...
  assert.equal(settings.combinedAnswerLanguage, 'English');
  ...
  assert.equal(settings.combinedPrompt, DEFAULT_COMBINED_PROMPT);
});
...
test('should normalize invalid settings without accepting unsupported values', () => {
  const settings = normalizeSettings({
    ...
    combinedModel: 'not-a-model',
    combinedReasoning: 'ultra',
    combinedImageDetail: 'not-an-image-detail',
    combinedAnswerLanguage: 'x'.repeat(100),
    combinedPrompt: '   '
  });
  ...
  assert.equal(settings.combinedModel, DEFAULT_SETTINGS.combinedModel);
  assert.equal(settings.combinedReasoning, DEFAULT_SETTINGS.combinedReasoning);
  assert.equal(settings.combinedImageDetail, DEFAULT_SETTINGS.combinedImageDetail);
  assert.equal(settings.combinedAnswerLanguage.length, 80);
  assert.equal(settings.combinedPrompt, DEFAULT_SETTINGS.combinedPrompt);
...
test('should persist settings outside the renderer state shape', () => {
  ...
  assert.equal(loaded.combinedModel, 'custom');
  assert.equal(loaded.combinedCustomModel, 'combined-model-123');
  assert.equal(loaded.combinedReasoning, 'high');
  assert.equal(loaded.combinedImageDetail, 'original');
  assert.equal(loaded.combinedPrompt, 'Use the screen carefully.');
  assert.equal(loaded.combinedAnswerLanguage, 'Polish');
